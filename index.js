 ...
 */
function loader(req, res, path, options, callback) {
 ...
 * @param callback {Function}
 */
function combine(paths, loader, options, callback) {
    var tpl, files = [];
    var meta = {
        mime: null,
        mtime: 0
    ...
    (function next(i, len) {
        if (i < len) {
            loader(paths[i], options, function (err, file) {
                if (err) {
                    callback(err);
                } else {
...
module.exports = function (options) {
    return function (req, res) {
        ...
        if (req.method() === 'GET' && (paths = parseURL(path))) { // Assign.
            ...
            combine(paths, load, options || {}, function (err, meta, data) {
